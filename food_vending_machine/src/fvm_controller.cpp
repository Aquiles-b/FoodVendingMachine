...
FvmController::FvmController(FvmDatabase& food_vm_db, asio::any_io_executor executor,
                                   NotificationCallback<ControllerEvent> client_notification)
    : m_fvm_db{food_vm_db}, m_executor{executor}, m_client_notification{client_notification},
      m_controller_state{FvmControllerState::MENU}, m_order{food_vm_db}
{
    m_fvm_payment = std::make_shared<FvmPayment>(
        m_executor, [this](PaymentEvent e) { payment_notification_callback(e); });
}

std::string FvmController::parse_user_input(std::string user_input)
{
    ...
    case FvmControllerState::MENU:
        return handle_menu_state(tokens);
    case FvmControllerState::PAYMENT:
...
std::string FvmController::handle_menu_state(const std::vector<std::string_view>& tokens)
{
...
std::string FvmController::handle_payment_state(const std::vector<std::string_view>& tokens)
{
    std::string response(tokens[0]);
    std::ostringstream out;

    if (!str_util::is_digit(response)) {
        switch (m_fvm_payment->get_payment_state()) {
        ...
        case PaymentState::WAITING_FOR_PAYMENT:
            out << "Error reading the card. Please try again:\n"
                << m_fvm_payment->request_payment(m_order.get_order_info().total_price);
            break;
    ...
    if (option == 0) {
        m_controller_state = FvmControllerState::MENU;
        m_fvm_payment->reset_payment_state();
        return "Payment canceled. Returning to menu.\n";
    }

    double money_input;
    std::vector<std::string> payment_options = m_fvm_payment->list_payment_options();
    ...
    case PaymentState::CHOOSING_PAYMENT_METHOD:
        ...
        out << m_fvm_payment->request_payment(m_order.get_order_info().total_price);
        break;
    case PaymentState::WAITING_FOR_PAYMENT:
        money_input = std::atof(response.data());
        out << "Processing payment...\n";
        m_fvm_payment->process_payment(m_order.get_order_info().total_price, money_input);
        break;
    case PaymentState::PROCESSING_PAYMENT:
        out << "Processing payment...\n";
        break;
...
void FvmController::payment_notification_callback(PaymentEvent e)
{
    ...
    case PaymentStatus::SUCCESS:
        m_order.register_event(FvmOrderStatus::PAYMENT_CONFIRMED);
        m_controller_state = FvmControllerState::MENU;
        m_fvm_payment->reset_payment_state();

        out << "Payment successful! Thank you for your purchase.\n";
        out << "Order #" << m_order.get_order_info().id << " is being prepared.\n";
        m_order.register_event(FvmOrderStatus::PREPARATION_STARTED);
    ...
    case PaymentStatus::INSUFFICIENT_FUNDS:
        m_order.register_event(FvmOrderStatus::PAYMENT_FAILED);
...
std::string FvmController::confirm_order()
{
    ...
    m_controller_state = FvmControllerState::PAYMENT;
...
std::string FvmController::get_payment_options_message()
{
    std::vector<std::string> payment_options = m_fvm_payment->list_payment_options();
    std::ostringstream out;
...
std::string FvmController::select_products(const std::vector<std::string_view>& selected_prods)
{
    std::unordered_map<std::string, ProductInfo> prod_name_mapped =
        m_fvm_db.get_product_info_name_mapped();
...
std::string FvmController::create_menu_table_message()
{
    std::ostringstream out;
    std::vector<ProductInfo> products = m_fvm_db.get_product_info();
